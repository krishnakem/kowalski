...
export class DebugOverlay {
     ...
     */
    async showTrajectory(from: Point, to: Point): Promise<void> {
        if (!this.enabled) return;
     ...
     */
    async highlightAllElements(elements: NavigationElement[]): Promise<void> {
    ...
    async showMessage(message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info'): Promise<void> {
        ...
        try {
            await this.page.evaluate(({ message, color }) => {
                const canvas = document.getElementById('kowalski-debug-canvas') as HTMLCanvasElement;
                if (!canvas) return;
